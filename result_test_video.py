...
checkpoint_path = "best_model.pth" if os.path.exists("best_model.pth") else "checkpoint.pth"

ckpt = torch.load(checkpoint_path, map_location=device)

eval_model = DiffusionUNet(2, 5, OBS_HORIZON).to(device)
eval_model.load_state_dict(ckpt['model'])
eval_model.eval()

print("Model loaded. First layer weight mean:", next(eval_model.parameters()).mean().item())
    ...
    for ep in tqdm(range(num_eval_episodes)):
        ...
        while not done and step < max_steps:
            ...
            for t in noise_scheduler.timesteps:
                ...
                noise_pred = eval_model(noisy_action, t_batch, obs_tensor)
                noisy_action = noise_scheduler.step(noise_pred, t, noisy_action).prev_sample
                
            pred_actions = noisy_action.squeeze(0)

            pred_actions = (pred_actions + 1.0) / 2.0 * (act_max - act_min) + act_min
...
print("\nGenerating visual demo for 1 episode...")
...
if len(frames) > 0:
    video_path = 'pusht_demo.mp4'
    with imageio.get_writer(video_path, format='FFMPEG', mode='I', fps=10) as writer:
          for frame in frames:
              writer.append_data(frame)
    print(f"Video successfully saved as {video_path}! ({len(frames)} frames)")
else:
